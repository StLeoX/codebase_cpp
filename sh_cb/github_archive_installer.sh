#!/usr/bin/env bash
# by StLeoX
# Downloader Template for GitHub Release

USER="StLeoX"
REPO="StLeoX"
REPO_TYPE="Rust"
SUPPORTED_REPO_TYPE="Rust \
                     JavaScript \
                     Python"
...
get_tmpfile() {
  ...
  suffix="$1"
  if has mktemp; then
    printf "%s%s.%s.%s" "$(mktemp)" "-glitter" "${RANDOM}" "${suffix}"
  else
...
download() {
  ...
  info "${BOLD}${UNDERLINE}https://github.com/${USER}/${REPO}/issues/new/${NO_COLOR}"
  return $rc
}
...
unpack() {
  ...
  info "bug report at https://github.com/${USER}/${REPO}/issues"
  return 1
}
...
case ${REPO_TYPE} in
  Rust)
    CARGO_TOML="$(curl -fsSL "https://raw.githubusercontent.com/${USER}/${REPO}/master/Cargo.toml")"
    ALL_VERSIONS="$(sed -n 's/.*version = "\([^"]*\)".*/\1/p' <<< "$CARGO_TOML")"
  ;;
  JavaScript)
    PACKAGE_JSON="$(curl -fsSL "https://raw.githubusercontent.com/${USER}/${REPO}/master/package.json")"
    ALL_VERSIONS="" # to do
esac
...
if [ -z "${BASE_URL-}" ]; then
  BASE_URL="https://github.com/${USER}/${REPO}/releases"
fi
